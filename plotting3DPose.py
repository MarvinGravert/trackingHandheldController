...
class Plotting3DPose(object):
    ...
    def start(self):
        ...
        def _animate(i):
            ...
            for pose in self.poseList:
                ...
                self.ax.plot([pose[0][0]],pose[0][1],pose[0][2],'ro',alpha=0.8, lw=3)
                print([pose[0][0]],pose[0][1],pose[0][2])
                #plot the axis of the system 
                r=R.from_quat(pose[1])
                xNew=r.apply(self.baseXVec)+pose[0]
                yNew=r.apply(self.baseYVec)+pose[0]
                zNew=r.apply(self.baseZVec)+pose[0]
                print(xNew) 
                print(xNew[1])             
    ...
    def update(self):
        ...
        for i in range(self.numPoses):
            ...
            self.poseList[i]=[[float(x) for x in userInput[0:3]],[float(x) for x in userInput[3:]]]
